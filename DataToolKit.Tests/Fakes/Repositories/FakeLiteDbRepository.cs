...
namespace DataToolKit.Tests.Fakes.Repositories
{
        ...
        where T : EntityBase
    {
        ...
        public void Write(IEnumerable<T> items)
        {
            ...
            // Delta-Detection via RepositoryDiffBuilder (now T : EntityBase)
            ...
            // Simulate Transaction (alle Operationen atomar)
            try
            {
                // Updates
            ...
            {
                // In einem Fake: Rollback = nichts tun (Operation ist atomar)
                throw;
            }
        }
        ...
        public void SeedData(params T[] items)
        {
            foreach (var item in items)
            {
                if (item.Id == 0)
                {
                    // Auto-ID: Verwende _nextId und inkrementiere
                    item.Id = _nextId++;
                }
                else
                {
                    // Explizite ID: Wenn sie >= _nextId ist, springe darüber hinaus
                    if (item.Id >= _nextId)
                        _nextId = item.Id + 1;
                }
                _items[item.Id] = item;
