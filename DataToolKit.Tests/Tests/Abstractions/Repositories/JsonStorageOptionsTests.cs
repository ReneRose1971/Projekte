...
{
    /// <summary>
    /// Tests für <see cref="JsonStorageOptions{T}"/>.
    /// Ziel: Korrekte Endung, Pfadbildung, automatische Verzeichnisanlage.
    /// Verwendet <see cref="TestDirectorySandbox"/> für saubere Test-Umgebungen.
    /// </summary>
    public sealed class JsonStorageOptionsTests : IDisposable
    {
        ...
        public JsonStorageOptionsTests()
        {
        ...
        [InlineData(null)]
        [InlineData("Module")]
        public void FullPath_EndsWith_DotJson(string? subFolder)
        {
            // Act
            var sut = new JsonStorageOptions<TestEntity>("TestApp", "data", subFolder, _sandbox.Root);

            // Assert
            Assert.EndsWith(".json", sut.FullPath, StringComparison.OrdinalIgnoreCase);
            ...
            var expected = System.IO.Path.Combine(effectiveRoot, "data.json");
        ...
        {
            // Act
            var sut = new JsonStorageOptions<TestEntity>("TestApp", "repo", subFolder, _sandbox.Root);
        ...
        [InlineData(null)]
        [InlineData("Nested")]
        public void MultipleOptions_ForDifferentTypes_HaveSamePath_WhenConfiguredIdentically(string? subFolder)
        {
            // Act
            var options1 = new JsonStorageOptions<TestEntity>("TestApp", "config", subFolder, _sandbox.Root);
            var options2 = new JsonStorageOptions<TestEntity>("TestApp", "config", subFolder, _sandbox.Root);
            ...
            Assert.EndsWith(".json", options2.FullPath, StringComparison.OrdinalIgnoreCase);
